...
B<Pod::Parser> is an abstract base class for implementing filters and/or
...
translation. The other thing that B<Pod::Parser> provides is the ability
to process only selected sections of pod documentation from the input.

=head2 SECTION SPECIFICATIONS

Certain methods and functions provided by B<Pod::Parser> may be given
...
$Pod::Parser::MAX_HEAD_LEVEL = 3;

##---------------------------------------------------------------------------

=head1 FUNCTIONS

B<Pod::Parser> provides the following functions (please note that these
...
B<Pod::Parser> provides several methods, some of which should be
...
if you want to create an instance of a B<Pod::Parser> instead of one of
...
sub select {
    ...
    for $spec (@sections) {
        ...
        @re = split('/', $_, $Pod::Parser::MAX_HEAD_LEVEL);
...
sub parse_paragraph {
    ...
    if (/^=((?:sub)*)(?:head(?:ing)?|sec(?:tion)?)(\d*)\s+(.*)\s*$/o) {
        ...
        my $i;
        for ($i = $level; $i < $Pod::Parser::MAX_HEAD_LEVEL; ++$i) {
            $self->{HEADINGS}->[$i] = '';
        }
    }
